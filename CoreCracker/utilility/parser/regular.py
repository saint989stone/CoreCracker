...
def serhHost(strg):   #функция поиску hostName на основании списка шаблонов
    ...
    if len(hostList):   #если список не пустой
        return hostList[-1]   #с целью избежания некорректного найденного шаблона отправляем последний найденный hostName (проблема UAK6-CR4-B - M7-CR9-A)
...
patnIntc = [
    'xe-\d/\d{1,2}/\d{1,2}',   #xe-1/33/22
    'et-\d/\d{1,2}/\d{1,2}',
    '100GE\d/\d{1,2}/\d{1,2}',   #100GE9/0/7
    'Gi\d/\d{1,2}/\d{1,2}:\d',     #'Gi\d/\d{1,2}/\d{1,2}:\d'
    'ae\d{1,2}', #ae45
    'Eth-Trunk\d{1,2}', #Eth-Trunk45
    'lag\d{1,2}'   #lag
]
